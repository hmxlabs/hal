...
| Canonical fixture usage | Integration should use canonical fixtures (`tests.md:72-82`, `test-matrix.md:50`) | Runner generates random prefixes (`tests/run-tests.sh:107`) and Hurl uses `{{test_prefix}}_*` IDs | Fixture naming/data differs from spec-defined baseline fixtures. |
...
| CI/report deliverable | Executable CI job with test report artifacts (`tests.md:137`) | No CI workflow or artifact-generation config found in repo | Deliverable appears missing. |

## Usage-matrix-specific divergences (Option 2 Python runner)

| Area | Usage matrix expectation | Implementation behavior | Divergence |
|---|---|---|---|
| CP observability side effects | Matrix emphasizes `/v1/content/*`, `/v1/events*`, `/v1/topology/*` assertions (`cache-usage-test-matrix.md:9`) | Runner can submit events but has no event-read verification path; assertions mostly use `nearest/locate/keys/route` (`tests/python/run_usage_tests.py:177-214`, `267-310`) | Event-stream outcomes in CP assertions are mostly not verifiable by the runner. |
